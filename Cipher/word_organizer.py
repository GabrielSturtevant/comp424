...
def organize_simple_list():
	read_file = open('./FormattedWordList.txt', 'r')
	write_file = open('./popular_words_copy.txt', 'w+')
	whole_file = read_file.read()
	whole_file = whole_file.replace("\'", "").replace('"', '')
	whole_file = whole_file.split('\n')
	for x in popular_words:
		whole_file.append(x)
	whole_file = list(sorted(set(whole_file)))
	whole_file = filter(None, whole_file)
	first = True
	for x in whole_file:
		if len(x) > 2:
			if first:
				var = 'popular_words = [\"{}\",\n'.format((x.rstrip()).upper())
				first = False
			elif x == whole_file[-1]:
				var = "\t\t\t\t \"{}\"]".format((x.rstrip()).upper())
			else:
				var = "\t\t\t\t \"{}\",\n".format((x.rstrip()).upper())
...
def organize_results():
	...
	file_var.close()
	file_var = open('./formatted_results.txt', 'w')
	for x in result_set:
		var = "{1}\n{0} -- Number of words\nWords:\n".format(x[0],x[1])
		for y in x[2].split(" "):
			if not y == "" and not y == " ":
